...
func exportMessageAsLine(e *fileExporter, message proto.Message) error {
	...
	var m2 = int(*rawdataUnmarshal.Transactions[0].StartTime)
	fmt.Printf("id lecture unmarshal  %v \n AgentID : %v \n", m, *rawdataUnmarshal.AgentId)
	fmt.Printf("startime 1st transction unmarshal  %v \n", m2)
...
func JSONtoRawdata(t AutoGenerated, rawdata *test.RawData) {
	...
	// initialisation, y ajouter les données, puis dire que rawdata.Transaction.Params = parametre
	...
	for _, v := range t.ResourceSpans[0].InstrumentationLibrarySpans[1].Spans {
		...
		// StartTime
		// Conversion
		StartTimeInt64, err := strconv.ParseInt(v.StartTimeUnixNano, 10, 64)
		fmt.Println("voici la valeur du starttime ", v.StartTimeUnixNano)
		if err != nil {
			fmt.Printf("%d of type %T", StartTimeInt64, StartTimeInt64)
		}
		// Mise en place dans un pointeur
		var StartTimePointer *int64 = &StartTimeInt64
		...
		for _, attr := range v.Attributes {
			var ParamPtr *test.Param
			var Param test.Param

			//
			var keyStringPointeur *string
			keyString := attr.Key
			keyStringPointeur = &keyString
			Param.Key = keyStringPointeur

			//
			var valueStringPointeur *string
			valueString := attr.Value.StringValue
			valueStringPointeur = &valueString
			Param.Value = valueStringPointeur

			ParamPtr = &Param
			Params = append(Params, ParamPtr)
...
// Function qui agrandit Transactions[] si il n'y a pas assez de place
